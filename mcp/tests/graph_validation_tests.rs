...
fn test_graph_invariants() {
    ...
    assert_eq!(invalid_callers, 0, "Found 'calls' edges originating from invalid assignment targets (variables, parameters, etc.)");
    ...
    let edge_exists: i64 = db.conn.query_row("SELECT COUNT(*) FROM edges WHERE kind = 'calls'", [], |r| r.get(0)).unwrap();
    if edge_exists > 0 {
        let (source, target): (String, String) = db.conn.query_row(
        ...
        for hop in &path.edges {
            let hop_exists: i64 = db.conn.query_row(
                "SELECT COUNT(*) FROM edges 
                 JOIN symbols s1 ON edges.source_symbol_id = s1.id 
                 JOIN symbols s2 ON edges.target_symbol_id = s2.id 
