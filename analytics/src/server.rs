...
async fn get_overview() -> Json<Value> {
    ...
    if let Ok(conn) = rusqlite::Connection::open(".codebroker/codebroker.db") {
        ...
        cache_hits = conn.query_row("SELECT COUNT(*) FROM mcp_analytics_events WHERE cache_hit = 1", [], |row| row.get(0)).unwrap_or(0);
        cache_misses = conn.query_row("SELECT COUNT(*) FROM mcp_analytics_events WHERE cache_hit = 0", [], |row| row.get(0)).unwrap_or(0);
        
        if let Ok(mut stmt) = conn.prepare("SELECT tool_name, COUNT(*) FROM mcp_analytics_events GROUP BY tool_name") {
