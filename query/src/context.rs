...
impl<'a> ContextResponseBuilder<'a> {
    ...
    pub fn fetch_external_imports(&self) -> Result<Vec<String>> {
        let mut ext_stmt = self.db.conn.prepare(
            "SELECT name FROM raw_imports WHERE file_id = ?1 AND is_local = 0"
        )?;
    ...
    pub fn fetch_prop_interfaces(&self) -> Result<Vec<crate::retrieval::SymbolSourceResult>> {
        ...
        let mut rows = stmt.query(rusqlite::params![self.file_id, like_props, like_props_lower])?;
        while let Some(row) = rows.next()? {
            let p_name: String = row.get(0)?;
            if let Ok(srcs) = crate::retrieval::read_symbol_source_scoped(self.db, &p_name, false, Some(&self.defining_file)) {
                props.extend(srcs);
            }
        }
        Ok(props)
    ...
    pub fn build_json(&self) -> Result<serde_json::Value> {
        ...
        map.insert("target_name".to_string(), serde_json::json!(self.target_name));
        map.insert("target_kind".to_string(), serde_json::json!(self.target_kind));
        map.insert("defining_file".to_string(), serde_json::json!(self.defining_file));
        map.insert("line_number".to_string(), serde_json::json!(self.line_number));
        ...
        let insert_vec = |map: &mut serde_json::Map<String, serde_json::Value>, key: &str, items: Vec<String>, b: &mut TokenBudget| {
            if items.is_empty() { return; }
            if !b.has_budget() { return; }
            let mut keep = Vec::new();
            for item in items {
                if b.consume(1) { keep.push(item); } else { break; }
            }
            if !keep.is_empty() {
        ...
        if self.profile == ResponseProfile::Verbose || self.profile == ResponseProfile::Standard {
            if let Ok(deps) = self.fetch_reverse_dependencies() { insert_vec(&mut map, "reverse_dependencies", deps, &mut b); }
            if let Ok(deps) = self.fetch_forward_dependencies() { insert_vec(&mut map, "forward_dependencies", deps, &mut b); }
            if let Ok(callers) = self.fetch_callers() { insert_vec(&mut map, "callers", callers, &mut b); }
            if let Ok(callees) = self.fetch_callees() { insert_vec(&mut map, "callees", callees, &mut b); }
            if let Ok(same) = self.fetch_same_file_callers() { insert_vec(&mut map, "same_file_callers", same, &mut b); }
        }

        if self.profile == ResponseProfile::Verbose {
            if let Ok(ext) = self.fetch_external_imports() { insert_vec(&mut map, "external_imports", ext, &mut b); }
            if let Ok(sib) = self.fetch_siblings() { insert_vec(&mut map, "siblings", sib, &mut b); }
            if let Ok(wrap) = self.fetch_wrapped_by() { insert_vec(&mut map, "wrapped_by", wrap, &mut b); }
            if let Ok(ren) = self.fetch_renders_components() { insert_vec(&mut map, "renders_components", ren, &mut b); }
            if let Ok(hooks) = self.fetch_consumes_hooks() { insert_vec(&mut map, "consumes_hooks", hooks, &mut b); }
        ...
        map.insert("graph_indexed".to_string(), serde_json::json!(self.graph_indexed));
    ...
    pub fn build_markdown(&self) -> Result<String> {
        ...
        if self.profile != ResponseProfile::Compact {
            ...
            if !self.decorators.is_empty() {
                md.push_str(&format!("**Decorators:** `{}`\n", self.decorators.join(", ")));
            }
        }

        let mut push_list = |md: &mut String, title: &str, items: Vec<String>, b: &mut TokenBudget| {
            if items.is_empty() { return; }
            if !b.has_budget() { return; }
            let mut keep = Vec::new();
            let total = items.len();
            ...
            if !keep.is_empty() {
                md.push_str(&format!("\n#### {}\n", title));
                for k in &keep { md.push_str(&format!("- {}\n", k)); }
                if keep.len() < total { md.push_str(&format!("- *...and {} more*\n", total - keep.len())); }
            }
        };

        if self.profile == ResponseProfile::Verbose || self.profile == ResponseProfile::Standard {
            if let Ok(callers) = self.fetch_callers() { push_list(&mut md, "Callers", callers, &mut b); }
            if let Ok(callees) = self.fetch_callees() { push_list(&mut md, "Callees", callees, &mut b); }
            if let Ok(same) = self.fetch_same_file_callers() { push_list(&mut md, "Same-File Callers", same, &mut b); }
            if let Ok(rev) = self.fetch_reverse_dependencies() { push_list(&mut md, "Reverse Dependencies (Importers)", rev, &mut b); }
            if let Ok(fwd) = self.fetch_forward_dependencies() { push_list(&mut md, "Forward Dependencies (Imports)", fwd, &mut b); }
        }

        if self.profile == ResponseProfile::Verbose {
            if let Ok(wrap) = self.fetch_wrapped_by() { push_list(&mut md, "Wrapped By", wrap, &mut b); }
            if let Ok(ren) = self.fetch_renders_components() { push_list(&mut md, "Renders Components", ren, &mut b); }
            if let Ok(hooks) = self.fetch_consumes_hooks() { push_list(&mut md, "Consumes Hooks", hooks, &mut b); }
            if let Ok(ext) = self.fetch_external_imports() { push_list(&mut md, "External Imports", ext, &mut b); }
            if let Ok(sib) = self.fetch_siblings() { push_list(&mut md, "Siblings", sib, &mut b); }
            
            if let Ok(props) = self.fetch_prop_interfaces() {
                if !props.is_empty() && b.consume(props.len() * 5) {
                    md.push_str("\n#### Prop Interfaces\n");
                    for p in props {
                        md.push_str(&format!("- `{}` ({} lines)\n", p.symbol_name, p.end_line.saturating_sub(p.start_line) + 1));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn same_file_caller_is_not_reported_as_dead_code() {
        ...
        let context = ContextResponseBuilder::new(&db, "generateRoomId", None, ResponseProfile::Verbose).unwrap().unwrap();

        assert!(
            context.fetch_reverse_dependencies().unwrap().is_empty(),
    ...
    fn unreferenced_symbol_has_no_same_file_callers() {
        ...
        let context = ContextResponseBuilder::new(&db, "GET", None, ResponseProfile::Verbose).unwrap().unwrap();

        assert!(context.fetch_same_file_callers().unwrap().is_empty(), "nothing in this fixture calls GET");
